...
soma_nivel_1 <- tb_aux_2008 %>%
  ...
  summarise(valor_mensal_total = sum(valor_mensal, na.rm = TRUE)) %>%
  rename(nivel = Nivel_1, descricao = Descricao_1) %>%
  mutate(indicador_nivel = 1)

soma_niveis_2008 <- soma_nivel_1
rm(soma_nivel_1)
soma_uc_2008 <- base_uc_2008_amzl %>% 
  group_by(descricao_estrato)  %>% 
  summarise(soma_uc_2008 = sum(PESO_FINAL,na.rm = TRUE))


tab_gastos_2008 <- merge(soma_niveis_2008, soma_uc_2008, by = "descricao_estrato", all.x = TRUE)
...
tab_gastos_2008 <- tab_gastos_2008 %>%
  mutate(
    gasto_mensal_familiar_2008 = round(valor_mensal_total / soma_uc_2008, 2)
  )
...
rm(soma_nivel_1)
soma_uc_2018 <- base_uc_2018_amzl %>% 
  group_by(descricao_estrato)  %>% 
  summarise(soma_uc_2018 = sum(PESO_FINAL,na.rm = TRUE))

tab_gastos_2018 <- merge(soma_niveis_2018, soma_uc_2018, by = "descricao_estrato", all.x = TRUE)
...
tab_gastos_2018 <- tab_gastos_2018 %>%
  mutate(
    gasto_mensal_familiar_2018 = round(valor_mensal_total / soma_uc_2018, 2)
  )

## Unindo as tabelas 
tab_gastos_combinada <- full_join(
  tab_gastos_2008 %>% select(descricao_estrato, nivel, descricao, indicador_nivel, gasto_mensal_familiar_2008),
  tab_gastos_2018 %>% select(descricao_estrato, nivel, descricao, indicador_nivel, gasto_mensal_familiar_2018),
  by = c("descricao_estrato","nivel", "descricao","indicador_nivel")
)


write.table(tab_gastos_combinada, paste(pathdir,"tabela_gastos_uc_2008_2018_amzlegal_8dec2024.csv", sep = ""),row.names = F, sep = ";")
